...
Since you're on NixOS, verify your `/etc/nixos/configuration.nix` has these EXACT lines:
...
networking.firewall = {
  ...
  allowedUDPPorts = [ 1716 ];  # Critical for discovery!
};
```

After adding/verifying, rebuild:
```bash
