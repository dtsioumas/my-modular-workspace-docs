...
**Plan Revised:** 2025-12-14 (Post-Ultrathink Review)
...
2. **Fixed incorrect assumptions** - nvidia-vaapi-driver already installed
3. **Lowered performance expectations** - Realistic codec support analysis
4. **Reordered Phase 8** - CUDA environment must come before Ollama
5. **Doubled time estimates** - Phase 7: 4-8hrs, Phase 8: 10-15hrs
6. **Added comprehensive testing** - Baseline capture, stress tests, rollback procedures
7. **Added Wayland configuration** - Missing MOZ_ENABLE_WAYLAND and related vars
8. **Added Electron apps optimization** - VSCode, Obsidian GPU acceleration
9. **Addressed VRAM resource conflicts** - Priority system and graceful degradation
10. **Enhanced risk mitigation** - Thermal monitoring, system backups, emergency procedures
...
**Codec Support Reality:**
- ✅ H.264 decode/encode (up to 4K)
- ⚠️ HEVC decode (8-bit only, no 10-bit, **NO encode**)
- ❌ VP9 decode/encode (NOT supported - YouTube uses this!)
- ❌ AV1 (NOT supported - Modern streaming uses this!)

**Impact:** Only ~20-30% of modern web videos will benefit from GPU decode (H.264 only).

### Revised Target State

```
GPU Utilization: 50-70% (from 14%) - Realistic given codec limitations
VRAM Usage: 65-75% (from 41%) - With resource management
CPU Offloading: 20-30% reduction for media, 10-15x for LLM inference
```

### Expected Benefits (Realistic)

1. **CPU Usage:** -20-30% on H.264 media, 10-15x speedup on LLM inference (3B models)
...
4. **Performance:** Faster LLM inference, smoother H.264 video playback
...
**Objective:** Resolve driver 570.x vs CUDA 11.0 compatibility question

**Critical Issue:** Driver 570.195.03 is CUDA 12.x era, but GTX 960 needs CUDA 11.0 runtime.
...
# NVIDIA Driver 570.x theoretically supports CUDA 11.0 through 12.8
# BUT: GTX 960 (compute 5.2) was dropped in CUDA 11.1+

# Decision tree:
# Option A: Keep driver 570.x, use CUDA 11.0 toolkit
#   - Risk: Unknown if 570.x works with CUDA 11.0 runtime
#   - Benefit: No driver downgrade needed
#   - Test: Install CUDA 11.0 toolkit, verify nvidia-smi shows it
#
# Option B: Downgrade to driver 470.x (last CUDA 11.0-focused driver)
#   - Risk: May break Wayland, KWin, existing apps
#   - Benefit: Guaranteed CUDA 11.0 support
#   - Require: Full system backup, test in VM first
```

**Research Needed:**
1. Check NVIDIA driver compatibility matrix for 570.x + CUDA 11.0
2. Search NixOS discourse/issues for GTX 960 + CUDA 11.0 experiences
3. Verify if runtime CUDA version must match driver era

**Decision Point:**
- [ ] **Option A (Recommended):** Keep 570.x driver, install CUDA 11.0 toolkit, test compatibility
- [ ] **Option B (Fallback):** Downgrade to 470.x driver if Option A fails

**If choosing Option B (downgrade):**
1. Create full system backup: `nixos-rebuild build`
2. Test in VM first with same configuration
3. Verify TTY access works (Ctrl+Alt+F2)
4. Document rollback procedure
5. Save all important work before reboot

**Checklist:**
- [ ] Research driver 570.x + CUDA 11.0 compatibility
- [ ] Decide on driver strategy (A or B)
- [ ] If Option B: Create system backup
- [ ] If Option B: Test driver change in VM
...
5. ✅ **Test in Firefox**
   ...
   - Check "Media" section → Look for:
     ...
     - "Hardware VP9 Decoding": **Unsupported** (expected, GTX 960 limitation)
   - Play YouTube H.264 video (look for older/480p content, or force H.264)
   - Run `nvidia-smi` to confirm GPU usage increases

**Codec Support Reality Check:**

```markdown
## GTX 960 Hardware Decode Support

| Codec | Support | YouTube Usage | Benefit |
|-------|---------|---------------|---------|
| H.264 | ✅ Full | ~20-30% of videos | High CPU reduction |
| HEVC 8-bit | ✅ Decode only | Rare on YouTube | Moderate benefit |
| VP9 | ❌ None | ~50-60% of videos | **NO benefit** |
| AV1 | ❌ None | ~10-20% (growing) | **NO benefit** |

**Reality:** Only H.264 content benefits from GPU decode.
Most modern YouTube videos (>1080p) use VP9 or AV1 → **CPU fallback**.

**How to find H.264 videos on YouTube:**
- Older videos (pre-2018)
- Lower resolutions (480p often H.264)
- Use browser extension to force codec
- Check Stats for Nerds (Ctrl+Shift+J) → "Codecs" line
```

**Expected Results (REVISED):**
- Firefox GPU usage: 8MB → 200-400MB (during H.264 video playback only)
- CPU usage during H.264 video: -40-50% ✅
- CPU usage during VP9 video: **No change** (0% improvement)
- Overall CPU reduction: **~20-30%** (weighted average, not 40-60%)
- VRAM total: 1686MB → 1900-2100MB
...
**Duration:** 10-15 hours (realistic estimate, was 4-6)
**Impact:** 10-15x speedup for LLM inference (3B models)
**Risk:** Medium-High (CUDA compatibility, driver issues)

**CRITICAL ORDER CHANGE:** Task 8.3 (CUDA environment) MUST come BEFORE 8.2 (Ollama)!
...
**Subtask 8.2.1: Install Ollama with CUDA 11.0**
...
{ config, pkgs, ... }:
{
  # Ollama service with CUDA 11.0 support
  services.ollama = {
    enable = true;
    acceleration = "cuda";

    # CRITICAL: Override to use CUDA 11.0 (not default CUDA 12.x)
    # NOTE: Verify this override syntax in Phase 0.2.2
...
4. ✅ **Verify service started**
   ...
   # Check if CUDA detected
   journalctl -u ollama -n 50
   # Look for: "CUDA available: true" or similar
   ```
5. ✅ **Download test model (3B)**
   ```bash
...
## Combined Phases 7-8: Expected Results (FINAL REALISTIC)

### Phase 7 Impact (Browser & Media)
- **Firefox GPU:** 8MB → 200-400MB (H.264 video only)
- **CPU reduction:** 20-30% weighted average (codec limited)
- **VRAM:** +300-500MB
- **GPU util:** 14% → 25-40%

### Phase 8 Impact (AI Agents)
- **LLM GPU:** 0% → 60-90% (when active)
- **VRAM:** +2-3GB (model in memory)
- **Inference speed:** 10-15x faster
- **GPU util:** 25-40% → 50-70% (when LLM active)

### Combined Total
- **GPU utilization:** 14% → 50-70% (realistic, was 60-80%)
- **VRAM utilization:** 41% → 65-75% (realistic, was 75-85%)
- **CPU offloading:** 20-30% for media, 90-95% for LLM
- **Power efficiency:** Better performance/watt on supported workloads
- **Faster workflows:** 10-15x LLM inference

### Realistic Expectations Set
- Not all YouTube videos will benefit (VP9/AV1 limitation)
