...
**Status:** ✅ Installed
**Configuration:** `~/.gemini/settings.json` (home-manager managed)
**Global Instructions:** `~/.gemini/AGENTS.md` (symlinked to llm-core)
...
### Method: Home-Manager (Declarative)

**Location:** `home-manager/gemini-cli.nix`
...
**Installed via:** `home-manager switch --flake .#mitsio@shoshin`
...
1. **Layer 1: Installation** (home-manager)
   - Package: `pkgs.gemini-cli`
   - MCP wrappers: All MCP servers packaged via ADR-010

2. **Layer 2: Configuration** (home-manager settings)
   - Settings: `~/.gemini/settings.json` (generated by home-manager)
   - Instructions: `~/.gemini/AGENTS.md` (symlinked to llm-core)
...
Key sections:
- `general`: Editor, session retention, auto-update (disabled)
- `ui`: Theme, line numbers, display options
- `model`: Default model, compression settings
- `privacy`: Telemetry disabled
- `tools`: Sandboxing, auto-accept, shell config
- `context`: AGENTS.md file location
- `mcpServers`: MCP server configurations (11+ servers)
