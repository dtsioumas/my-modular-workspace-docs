...
- `dotfiles/private_dot_config/powerdevilrc.tmpl` – AC display/suspend settings rendered from `.chezmoidata/plasma.yaml`.
- `dotfiles/private_dot_config/plasmanotifyrc.tmpl` – Notification defaults (popup position, DND, history apps) from `.chezmoidata/plasma.yaml`.
- `dotfiles/private_dot_config/krunnerrc.tmpl` – KRunner behavior rendered from `.chezmoidata/plasma.yaml`.
- `dotfiles/private_dot_config/plasma_workspace.notifyrc.tmpl` – Device-added/removed + critical message actions from `.chezmoidata/plasma.yaml`.
...
3. Run `make plasma-refresh` ... (Templated files such as `kscreenlockerrc`, `plasma-localerc`, `plasma_calendar_holiday_regions`, `plasmarc`, `powerdevilrc`, `plasmanotifyrc`, `krunnerrc`, and `plasma_workspace.notifyrc` are excluded.)
